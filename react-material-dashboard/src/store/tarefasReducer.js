...
export const tarefaReducer = (state = ESTADO_INICIAL, action) => {
  switch(action.type){
    case ACTIONS.LISTAR: 
      return{...state, tarefas: action.tarefas, quantidade: action.tarefas.lenght}
  ...
  return dispatch=>{
    http.delete(`./tarefas/${id}`,{
      ...
      dispatch([{
        ...
        id:id
      }, mostrarMensagem('Tarefa Deletada!')])
    })
  }
}
  ...
  return dispatch => {
    http.patch(`tarefas/${id}`, null, {
      ...
      dispatch([{
        ...
        id:id
      }, mostrarMensagem('Tarefa Alterada!')])
    })
  }
}